...
export default function Modal({ isOpen, onClose }) {
  ...
  return (
    <div
      className={`fixed top-0 left-0 w-full h-full bg-custom-dark z-[999] ${isOpen ? "flex" : "hidden"
        } md:items-center md:justify-center`}
      onClick={onClose}
    >
      <div
        className="md:max-w-[1031px] w-full relative md:rounded-2xl overflow-hidden flex items-center md:bg-[#24243E] bg-[#1A1A2E] md:flex-row flex-col overflow-y-auto"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          className="absolute top-5 md:right-6 right-[15px] z-[100]"
          onClick={onClose}
        >
        ...
        <div className="bg-[#1A1A2E] md:w-auto w-full md:h-full md:min-h-fit min-h-[450px] overflow-hidden md:flex-1 relative z-10 md:pb-[50px] md:px-[34px] md:pt-[18px]">
          ...
          <div className="md:mt-[72px] mt-[30px] mb-6 md:px-0 px-[15px]">
            ...
            <button className="yellow-btn md:mt-[35px] mt-3 md:!h-[54px] !h-[42px]">
              Add to trade
            </button>
          </div>
        </div>
        <div className="lg:w-[472px] h-fit md:w-[380px] w-full px-[15px] md:px-[31px] md:py-5 pb-8">
          <div className="space-y-[27px]">
            ...
            <Link
              className="md:hidden flex items-center justify-center gap-[10px] h-[50px] w-full bg-[#615FA6] rounded-[9px]"
              to="#!"
            >
